...
/* to aquire data from sonar interface board */
#define BAUDRATE B9600          
#define MODEMDEVICE "/dev/ttyS1"
/* serial port setup config structs */
struct termios oldtio,newtio;
/* global file descriptor for serial device */
int  fd;
...
private:
	...
	/* serial device communication specific variables*/
 	int res;		// no of bytes read from serial device
        char *intro;		//packets header
        char buf[30]; 		// RX buffer
   	char *cp;		// copy of the RX buffer
	// range information ( in cm )
	long val_s1, val_s2, val_s3, val_s4, val_s5;// decimal values read from sonars 
	// delimiter for sonar reads in the packet data field
  	char *delimiters;
	// modem control 
  	int mcr; 
	// files to store odometry and sonar data
	FILE *saved_pose;
	FILE *saved_readings;
...
public:
	...
	// odometry specific variables initialization
		...
		// delimiter for sonar values in serial packets data field
		delimiters = ",";
		mcr = 0;
		val_s1 = val_s2 = val_s3 = val_s4 = val_s5 = 0;
		// open and setup serial port
		open_and_setup_ser_dev();
		// open the file to write sonar read values
		saved_readings = fopen("/mnt/nfs/saved_readings.mat","w+");
		// open the file to write odometry data 
		saved_pose = fopen("/mnt/nfs/saved_pose.mat","w+");
...
// open and setup the serial connection to sonar interface board
inline void open_and_setup_ser_dev()
{ 
	// open device and test for errors
	 fd = open(MODEMDEVICE, O_RDWR | O_NOCTTY ); 
         if (fd <0) {perror(MODEMDEVICE); exit(-1); }
         // get current serial device attributes
         tcgetattr(fd,&oldtio); 
	  // new  termios handler struct
         bzero(&newtio, sizeof(newtio)); 
         // setup baudrate, RTS-CTS, setup char size
         newtio.c_cflag = BAUDRATE | CRTSCTS | CS8 | CLOCAL | CREAD;
         // ignore parity, 
         newtio.c_iflag = IGNPAR | ICRNL;
	 // clear ouput control flags
         newtio.c_oflag = 0;
         // canonical input processing
         newtio.c_lflag = ICANON;
         // communication special characters setup       
   ...
   tcflush(fd, TCIFLUSH);
   // set the new attributes for the connection
...
void control(){
	...
	res = read(fd,buf,30); 
        buf[res]=0;   
 	// copy the buffer for further processing          
        cp = strdup(buf);
	// check packet header
	intro = strtok(buf, delimiters);
	// extract info from the data field
	...
	// print for debugging
	// TODO formatting mode
        fprintf(saved_readings,"%ld | %ld | %ld | %ld | %ld \n", 
		val_s1, 
		val_s2, 
		val_s3, 
		val_s4, 
...
if(execute%4==0){
	...
	// set point value setting for PI controllers as output from SM controller

	if(inject_fault!=1 && inject_fault!=2 && inject_fault!=3 && inject_fault!=4){
		w_l_des = (v - WHEEL_BASE*w/2)/WHEEL_RADIUS;
		w_r_des = (v + WHEEL_BASE*w/2)/WHEEL_RADIUS;
		}
		else{
			switch(inject_fault){
			case 1 : 
				
				if(increment>=INJECT_F1+40){
...
void end_ctrl(){
		...
		sens = 0.0; // 0 forward and 1 for backward
		reset_parameters();
		// reset serial device interface parameters
	...
	double min3 = temp[0][2];
		...
		for(int i = 0;i<4;i++){
			  ...
			  // if((min1==r1[1] && min2 ==r1[2] || min2==r1[2] && min3 ==r1[3] || min1==r1[1] && min3 ==r1[3])|| (min1==r2[1] && min2 ==r2[2] || min2==r2[2] && min3 ==r2[3] || min1==r2[1] && min3 ==r2[3])){
			        if(inject_fault==1.0||inject_fault==2.0){
