function v_finalll=algoritm_feb_sep_01(v_fin)
...
for g=1:length(P)-1
    ...
    while aW > 0.20 %0.31 0.25
        ...
        V=algorith_velo_fin(xA,yA,xB,yB,LLL,tfin,v_in,v_fin(g),0,0,tetaA,l,KK,alfa,beta);     
...
figure(2)
  %  subplot(2,1,1)
        ...
        ylabel('Viteza','fontsize',12)
        legend('Viteza liniara a robotului [m/s]','Viteza unghiulara a robotului [rad/s]')
%     subplot(2,1,2)
...
% ajustare pass de calcul pentru viteze initial pas increment =  10 -->
% 100ms
%    speed = velocity(2,1:10:length(velocity))';
%   acceler = velocity(3,1:10:length(velocity))';
%   angular_velo = velocity(6,1:10:length(velocity))';
  
  % ajustare pass de calcul pentru viteze actual pas increment = 1 -->
  % mai multe puncte intermediare
  % 20 -- 200 ms
  % 50 -- 500 ms
   speed = velocity(2,1:20:length(velocity))';
   acceler = velocity(3,1:20:length(velocity))';
   angular_velo = velocity(6,1:20:length(velocity))';
  ...
  date2 = [speed, angular_velo];
   ...
   save acceler acceler -v4;
       ...
       fid = fopen('D:\school_stuff_and_work\DIZERTATIE\RTAI_robot_app\ROBOT TRAJECTORY GENERATOR\Velocity\slam_test_trajectory.h','wt');
        % for linux 
        %fid = fopen('/media/MOBILEDISK/ROBOT TRAJECTORY GENERATOR/Velocity/trajectory_planner_demo_circuit_complex.h','wt');
        
        fprintf(fid,'double len=%d; \n',length(speed));
        fprintf(fid,'static double ref_profile[][4]={ \n');
